...
const sessionFromQuery = computed(() => route.query.session as string || '')
...
onMounted(async () => {
  if (mode.value === 'twenty-q' && sessionFromQuery.value) {
    // 二十问结果
    try {
      const data = await api.getTwentyQResult(sessionFromQuery.value)
      answer.value = data.answer
      score.value = data.score
      isCorrect.value = data.correct
      stars.value = data.correct ? (data.remaining_questions >= 15 ? 5 : data.remaining_questions >= 10 ? 4 : data.remaining_questions >= 5 ? 3 : 2) : 1
...
async function replay() {
  if (mode.value === 'twenty-q') {
    router.push('/twenty-q')
  } else {
    game.reset()
...
.result-circle {
  ...
  box-shadow: 0 0 30px var(--diablo-gold-glow); position: relative;
}
